...
- Groq LLM      -> USD per 1M prompt / completion tokens (from `usage`).
...
# ---- Groq LLM (USD per 1M tokens) ------------------------------------------
# openai/gpt-oss-120b on Groq: ~$0.15 in / $0.75 out per 1M tokens.
GROQ_IN_PER_1M = float(os.getenv("RATE_GROQ_IN_PER_1M", "0.15"))
GROQ_OUT_PER_1M = float(os.getenv("RATE_GROQ_OUT_PER_1M", "0.75"))
...
def _empty_usage():
    ...
    return {
        ...
        "llm_in": 0,          # Groq prompt tokens
        "llm_out": 0,         # Groq output tokens
...
def compute_ai_cost(usage):
    ...
    llm = ((u.get("llm_in", 0) / 1_000_000.0) * GROQ_IN_PER_1M
           + (u.get("llm_out", 0) / 1_000_000.0) * GROQ_OUT_PER_1M)
...
def ai_cost_breakdown(usage):
    ...
    llm_in = round((u.get("llm_in", 0) / 1e6) * GROQ_IN_PER_1M, 6)
    llm_out = round((u.get("llm_out", 0) / 1e6) * GROQ_OUT_PER_1M, 6)
    ...
    return {
        "usage": u,
        "providers": {"stt": "deepgram", "llm": "groq", "tts": "sarvam-bulbul"},
